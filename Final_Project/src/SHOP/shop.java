...
public class shop {
	...
	private void Select_Displayed_Items() {
		displayedItems.clear(); // clear existing display of items 
	...
	private Item Get_Random_BoosterPack() {
		...
		if ( chance < /* some value */ ) {
			return new boosterpack (/* BRONZE*/);
			// repeat for other boostpack values make it where it is garenteed 1 like if everythign fails then hit this type thing 
		}
	}
	private Item Get_Random_Single_Card() {
		Random random = new Random();
		int value = random.nextInt(10)+1; // generate random value between 1 and 10
		return new SingleCard(/* whate ver it is assign Value here */);
	}
	private Item Get_Random_Item() {
		Random random = new Ramom();
